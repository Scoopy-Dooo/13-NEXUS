...
export default function AsideLink({ icon, text, color }) {
    ...
    return <NavLink to={"/" + (text == "profile" ? ("profile/" + userData?._id) : text)} className="lg:w-full ">
        {({ isActive }) => (
            <div className={`
                ...
                <div className={`absolute transition-all w-1 lg:w-1.5 h-full left-0 ${isActive ? "bg-linear-to-b  from-pink-700 to-50% to-indigo-800" : "bg-transparent "} `}></div>
                ...
                <h3 className='text-inherit font-semibold max-lg:hidden text-lg capitalize'>{text}</h3>
            </div>
        )}
    </NavLink >
}